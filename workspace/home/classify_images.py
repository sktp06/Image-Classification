...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
        # Get the full path to the image by combining images_dir with the filename
        ...
        # Extract the pet image label from the results_dic
        pet_label = results_dic[key][0]
        
        # Check if the pet image label is found within the classifier label
        match = 1 if pet_label in classifier_label else 0
        
        # Update the results_dic for each key with the classifier label and comparison
